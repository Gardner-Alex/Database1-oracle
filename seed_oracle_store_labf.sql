...
@/home/student/Data/cit225/oracle/lab4/seed_oracle_store.sql
...
( street_address_s1_lab.nextval
, address_s1_lab.currval
,'10 El Camino Real'
, 2, SYSDATE, 2, SYSDATE);
...
,(SELECT   common_lookup_lab_id
  FROM     common_lookup_lab
  WHERE    common_lookup_lab_type = 'VHS_SINGLE_TAPE')
...
,(SELECT   common_lookup_lab_id
  FROM     common_lookup_lab
  WHERE    common_lookup_lab_type = 'VHS_DOUBLE_TAPE')
...
,(SELECT   d.item_lab_id
  FROM     item_lab d
  ,        common_lookup_lab cl
  WHERE    d.item_lab_title = 'I Remember Mama'
  AND      d.item_lab_subtitle IS NULL
  AND      d.item_lab_type = cl.common_lookup_lab_id
  AND      cl.common_lookup_lab_type = 'VHS_SINGLE_TAPE')
, 3, SYSDATE, 3, SYSDATE);
...
,(SELECT   d.item_lab_id
  ...
  AND      d.item_lab_subtitle IS NULL
  AND      d.item_lab_type = cl.common_lookup_lab_id
  AND      cl.common_lookup_lab_type = 'VHS_DOUBLE_TAPE')
, 3, SYSDATE, 3, SYSDATE);
...
UPDATE member m
SET    member_type = (SELECT   common_looku_labp_id
                      FROM     common_lookup_lab
                      WHERE    common_lookup_lab_context = 'MEMBER'
...
FROM     member m INNER JOIN contact_lab c
...
BEGIN
  ...
  ,(SELECT   common_lookup_lab_id
    FROM     common_lookup_lab
    WHERE    common_lookup_lab_context = 'MEMBER'
    AND      common_lookup_lab_type = lv_member_type)
  , pv_account_number
  ...
  ,(SELECT   common_lookup_lab_id
    ...
    AND      common_lookup_lab_type = lv_address_type)
  , pv_city
  , pv_state_province
  , pv_postal_code
  , pv_created_by
  ...
  , address_s1_lab.CURRVAL                                -- ADDRESS_ID
  ,(SELECT   common_lookup_lab_id                         -- ADDRESS_TYPE
    FROM     common_lookup_lab
    WHERE    common_lookup_lab_context = 'MULTIPLE'
    AND      common_lookup_lab_type = lv_telephone_type)
